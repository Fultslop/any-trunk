...
async function activateStore(store, service) {
  ...
  try {
    ...
    navigate('space-list', {
      store, service, registrySpaceId, services: SERVICES,
    });
  } catch (error) {
    container.innerHTML = `
      <div class="text-center py-16">
        <p class="text-red-600 mb-4">Could not connect to your storage: ${error.message}</p>
...
async function start() {
  ...
  if (!store && service.id === 'local') {
    ...
    document.querySelector('#local-connect').addEventListener('click', async () => {
