...
export class WorkerGitHubStore extends GitHubStore {
  ...
  constructor({
    clientId, workerUrl, token = null, repoFullName = null, _username = null,
  } = {}) {
    super({
      clientId, token, repoFullName, _username,
    });
  ...
  static async init({
    clientId, workerUrl, repoFullName = null, mode = null,
  } = {}) {
    const parameters = new URLSearchParams(location.search);
    const code = parameters.get('code');
  ...
  async register() {
    const resp = await fetch(`${this.#workerUrl}/spaces/register`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ repo: this.spaceId, token: this.token }),
    });
    if (!resp.ok) throw new Error(`register failed: HTTP ${resp.status}`);
    const { inviteCode } = await resp.json();
    localStorage.setItem(`gifts:${encodeURIComponent(this.spaceId)}:inviteCode`, inviteCode);
    return inviteCode;
  }
  ...
  // Calls Worker /spaces/invite; then unconditionally calls autoAcceptInvitation
  async join(repoFullName, inviteCode) {
    this.spaceId = repoFullName;

    const resp = await fetch(`${this.#workerUrl}/spaces/invite`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ repo: repoFullName, username: this.username, inviteCode }),
    });
    if (!resp.ok) throw new Error(`join failed: HTTP ${resp.status}`);

    // Always attempt auto-accept — autoAcceptInvitation returns silently if
